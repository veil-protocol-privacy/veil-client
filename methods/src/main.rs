...
fn main() {
    ...
    let mut message_data: Vec<&[u8]> = vec![args.public_data.merkle_root.as_slice(), args.public_data.params_hash.as_slice()];
    message_data.extend(args.public_data.nullifiers.iter().map( |nullifier| nullifier.as_slice()));
    ...
    let master_pubkey = poseidon(vec![&args.private_data.pubkey, &nullifying_key]);
    
    args.public_data.nullifiers.iter().enumerate().for_each(|(i, nullifier)| {
        let leaf_index = args.private_data.merkle_leaf_indices[i].to_le_bytes();
        let nullifier_hash = poseidon(vec![nullifying_key.as_slice(), &leaf_index]);
    ...
    for i in 0..input_count {
        ...
        let utxo_key_in = poseidon(vec![&master_pubkey, random]);
        let amount_in = args.private_data.amount_in[i];
        assert!(amount_in != 0);

        let input_hash = poseidon(vec![
            &utxo_key_in, &token_id, &amount_in.to_le_bytes(),
        ]);
        
        let leaf_index = args.private_data.merkle_leaf_indices[i];
        merkle_proof_check(input_hash, leaf_index, root.clone(), args.private_data.merkle_paths[i].clone(), args.tree_depth);
    }

    args.public_data.output_hashes.iter().enumerate().for_each(| (i, output_hash) |  {
        let amount_out = args.private_data.amount_out[i].to_le_bytes();
        let hash = poseidon(vec![&args.private_data.utxo_output_keys[i], &token_id, &amount_out]);
