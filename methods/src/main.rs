...
use types::{
    utils::{hash_left_right, poseidon},
    Arguments,
};
...
fn main() {
    ...
    let pubkey = VerificationKey::try_from(args.private_data.pubkey.as_slice()).unwrap();
    // let signature = Signature::from(<[u8; 64]>::try_from(&args.private_data.signature[..]).unwrap());
    let signature = Signature::try_from(args.private_data.signature.as_slice()).unwrap();

    let err = pubkey.verify(&signature, &message_hash).err();
    if err.is_some() {
        panic!("{}", err.unwrap());
    };
    ...
    for i in 0..input_count {
        ...
        let input_hash = poseidon(vec![&utxo_key_in, &token_id, &amount_in.to_le_bytes()]);
