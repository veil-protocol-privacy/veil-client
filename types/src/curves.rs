...
pub fn blind_keys(
    ...
    let blinding_scalar = Scalar::hash_from_bytes::<Sha512>(&random.as_slice());
    let sender_pubkey_compressed = CompressedEdwardsY::from_slice(&sender_viewing_pubkey);
    let sender_pubkey_point = sender_pubkey_compressed.decompress().unwrap();

    let receiver_pubkey_compressed = CompressedEdwardsY::from_slice(&receiver_viewing_pubkey);
...
pub fn share_key(private_key: Vec<u8>, public_key: Vec<u8>) -> Vec<u8> {
    ...
    let public_key_compressed = CompressedEdwardsY::from_slice(&public_key);
    let public_key_point = public_key_compressed.decompress().unwrap();

    (public_key_point * private_key_scalar)
        .compress()
        .as_bytes()
        .to_vec()
}
