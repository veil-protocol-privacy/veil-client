...
use crate::{blind_keys, poseidon, share_key, CipherText, PlainText};
...
impl UTXO {
    pub fn new(spending_key: Vec<u8>, viewing_key: Vec<u8>, token_id: Vec<u8>, random: Vec<u8>, nonce: Vec<u8>, amount: u64, memo: String) -> Self {
        Self {
            spending_key,
            viewing_key,
            random,
    ...
    pub fn encrypt(
        ...
        let (blinded_sender_pubkey, blinded_receiver_pubkey) = blind_keys(sender_viewing_pubkey, self.viewing_public_key(), self.nonce.clone());
    ...
    pub fn decrypt(
        ...
        let plain_text = match PlainText::try_from_slice(&decrypted_data) {
            Ok(text) => text,
            Err(err) => panic!("decrypted data not match with plain text structure: {}", err)
        };
...
mod tests{
    ...
    pub fn encrypt_aes(
        ...
        CipherText::new(ciphertext, random, blinded_sender_pubkey, blinded_receiver_pubkey)
    }
